<?php
    ...
    if($type == 'AM')
    {
        ...
        $j = str_replace('"', "'", $j);
        $result = pg_query($conn,"SELECT * FROM collection where lower(client_name) = any(array$j) order by id");
    ...
    else if($type == 'RM')
    {
        ...
        $result = pg_query($conn,"SELECT * FROM collection WHERE lower(account) = ANY('"."{".$accounttype."}"."')");
        $arr_collection = pg_fetch_all($result);
        while($row = pg_fetch_assoc($result)) {
            $history = pg_query($conn, "SELECT * FROM revised_history where meta_id=".$row["id"]."");
    ...
    else if($type == 'BU')
    {
        ...
        $result = pg_query($conn,"SELECT * FROM collection where bu = ANY('" . "{" . $bu . "}" . "'::text[]) order by id");
    ...
    {
        $result = pg_query($conn,"SELECT * FROM collection order by id");
        $arr_collection = pg_fetch_all($result);
        $history_permission = false;
    }
    else if($type == 'ADMIN')
